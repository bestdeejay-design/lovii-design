...
## 8. LOVII PAY — профиль лояльности (v1.13.2; скины v1.14.0)

Программа **LOVII PAY**: карта + счёт + баланс. Термины владельца (2026-09): платформа — «Лови», программа — LOVII PAY; счёт и баланс — составляющие LOVII PAY. «Клубов» и «карты жителя» нет. Экономика: 1 балл = 1 ₽, баллы не сгорают, вывод через СБП (canon VISION.md). Карты (v1.14.0, канон 2026-09-12): у клиента ОДИН счёт — скины **LOVII PAY** (база, всем при регистрации) → **LOVII PASS** (подписка; статус представителя + вывод на карту 0% от 3 000 ₽) → **LOVII VIP** (оборот от 300 000 ₽ в месяц); **LOVII BUSINESS** — бизнес-карта МСП (свой счёт точки, операции по нему — в профиле). Привилегии — мерч, мероприятия и спец-скидки у МСП; повышенный кэшбек не используется.

**Композиция экрана (v1.13.2, адаптив):** контейнер `.lv-narrow` (max 760px). Мобильный (<640px): один столбец в порядке карта → счёт → история → статус → привилегии → МСП. От **≥640px** — двухколоночная сетка `.pay-cols` (5fr/6fr) «как в веб-банке»: слева карта + счёт (`.pay-col-a`, sticky top 68px при высоте окна ≥660px), справа история + статус (`.pay-col-b`); привилегии — сеткой `auto-fit minmax(200px,1fr)`, от 1024px — промежуток 28px. Порядок секций на мобильном не меняется — колонки стекаются. Выравнивание в колонках (v1.13.3): карточки (`.tx-list`, `.tier-card`, `.acct-card`) — без мобильных боковых margin, по кромкам колонки; первый заголовок правой колонки — по верхней кромке карты; табы фильтров — `nowrap` 11.5px (в колонке 320px все четыре без обрезки); подсказки под картой переносятся целыми строками. Внутренний отступ контента карточки истории (v1.13.4): `.pay-cols .tx-list { padding: 2px 14px }` — боковые margin у карточки сброшены, но паддинг контента сохранён (как на мобильном), иначе иконки операций `.tx-ico` липнут к рамке.

### Карта `.paycard` + `.pay-*`

| Часть | Анатомия | Правила |
|---|---|---|
| Лицевая сторона | brand-слово `LOVII PAY/PASS/VIP/BUSINESS` (градиентный текст по скину) · чип 40×30 · номер 4-4-4-4 (стандарт Visa/Мир, 16 цифр) · держатель · счёт (gold, 16/800) | фон и акценты — по скину (см. «Скины карт»); базовая окантовка `inset 0 0 0 1px rgba(255,255,255,.09)` — сепарация на тёмной теме |
| Оборот | магнитная полоса · QR 84×84 (оплата у партнёров; у бизнес-карты — QR точки) · CVV-плашка · подпись «1 балл = 1 ₽ · баллы не сгорают · счёт один — скины меняются» | CVV демо-маской `•••` |
| Движение | flip `rotateY(180deg)` по тапу, .65s cubic-bezier(.2,.75,.25,1) · tilt по курсору ±10°/±8° · блик `.pay-glare` — световое пятно следит за курсором через `--gx/--gy` (только `pointer: fine`; off при reduce-motion) | `aspect-ratio: 1.586/1`, радиус `--lv-r-xl`, тень `--lv-shadow-nav`; на hover тень глубже (`@media hover:hover`) |
...
### Скины карт (v1.14.0 — канон владельца 2026-09-12)

У клиента внутри платформы **ОДИН счёт и ОДИН номер** — статус меняет только скин (оформление). Класс скина ставится на `.paycard`, действует на обе стороны. Превью в профиле — клик по уровню в `.tier-levels` (кнопки, повторный клик возвращает свой скин).

| Скин | Класс | Кому выдаётся | Подача |
|---|---|---|---|
| LOVII PAY | `.skin-pay` | всем при регистрации (база) | матовый графит #35303b→#1b1721, серебряные brand/чип, слабый белый sheen |
| LOVII PASS | `.skin-pass` | по действующей подписке Лови; даёт **статус представителя LOVII** и **вывод баллов на карту без комиссий от 3 000 ₽** | базовый ink + розовая кромка/свечение бренда, brand-градиент pink→gold |
| LOVII VIP | `.skin-vip` | оборот LOVII **от 300 000 ₽ в месяц** | чёрное золото #3a2f14→#14100c, золотая кромка, золотой номер, тёплый glare |
| LOVII BUSINESS | `.skin-biz` | по умолчанию всем МСП (торговым точкам); счёт у точки свой, в профиле — все операции по нему | графит + тиффани #17262f→#0d171d, тиффани brand/чип, brand 14px (длинное слово) |

Правила вывода: LOVII PASS и выше — «Вывести на карту · 0% от 3 000 ₽» (кнопка `.acct-btn brand` меняет подпись); для базового PAY правила вывода не заданы владельцем (беклог `BACKLOG.md §6`, числа не выдумывать).
...
| `.tier-*` | Статус LOVII PAY: полоска уровней `.tier-levels` (кнопки-скины PAY→PASS→VIP, текущий — gold-капсула, превью — розовое кольцо `.pv`) + подпись `.pay-skin-note` («счёт один — скин меняется со статусом») + имя (crown gold) + условие уровня + прогресс-бар (gradient-gold) + перки-чипы | бар h6 pill; прогресс: подписка — бинарно (0/100%), VIP — месячный оборот/порог 300 000 ₽ |
| `.priv-card` | Витрина привилегий: плитка 168px, иконка 34×34 в soft-паре (gold/pink/tiffany) | <640px — лента `.hscroll`; ≥640px — сетка `auto-fit minmax(200px,1fr)`, плитка auto |

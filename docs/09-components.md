...
## 8. LOVII CLUB · LOVII PAY — клубный профиль (v1.13.0)

Клуб «Лови» (**LOVII CLUB**, LC) и карта участника (**LOVII PAY**). Термины владельца: платформа — «Лови», клуб — LOVII CLUB, карта — LOVII PAY; счёт и баланс — составляющие LOVII PAY. «Карты жителя» нет. Экономика клуба: 1 балл = 1 ₽, баллы не сгорают, вывод через СБП (canon VISION.md).
...
Держатель и номер — `tabular-nums`, без переноса (`white-space: nowrap`); номер ужимается clamp(17px→21px). Префикс (BIN) — прерогатива генератора номеров (отдельная задача), в демо — константа.

### Счёт, история, статус, привилегии

| Компонент | Назначение | Ключевые значения |
|---|---|---|
| `.acct-*` | Счёт кэшбека: баланс 30/800, 3 метрики (за месяц / ставка / выведено), действия «Вывести через СБП» (brand) + «История» (ghost) | карточка `--lv-r-lg`, кнопки h44 капсулы |
...
| `.tier-*` | Статус LC: имя уровня (crown gold) + ставка + прогресс-бар (gradient-gold) + перки-чипы | бар h6 pill; уровни Start → Silver → Gold → Ink |

...
14. [Навигация ДС (nav.js)](#14-навигация-дс-navjs)
...
```
Do: один переиспользуемый элемент; на брендовом градиенте вторичный текст `--lv-toast-d`. Канонические тексты тостов (успех/ошибка/сеть) — docs/11 §5; тон по ситуации — матрица docs/11 §2.
...
Do: формула «иконка + заголовок + тело с перспективой + кнопка» обязательна; маскот `i-bear` — только в дружелюбных сценах. Канонические формулировки (заголовок/тело/кнопка для витрины, поиска, заказов) — docs/11 §5; чек-лист текста — docs/11 §8 и checklist §12.
...
## 14. Навигация ДС (nav.js)

Сквозное меню между всеми страницами дизайн-системы (хаб, гайд, референсы). Один файл — `assets/nav.js`; сам находит хедер (`.app-header` / `.gs-head-right`), добавляет кнопку «☰», а если хедера нет — рисует плавающую кнопку. Шит — по канону §9.

```html
<!-- последним перед </body>; на страницах в examples/ путь ../assets/nav.js -->
<script src="assets/nav.js" defer></script>
```

Do: подключать на каждой странице ДС; активный пункт подсвечивается автоматически («вы здесь»); закрытие по оверлею/Escape; кнопка 36px как theme-btn.
Don't: дублировать своё меню на страницах ДС; править список страниц внутри страниц — только в `assets/nav.js` (массив `ITEMS`).

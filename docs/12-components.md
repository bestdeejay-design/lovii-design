...
Модальные диалоги по центру экрана (в каноне — bottom-sheet), dropdown-меню (в каноне — шиты и rows), таблицы данных (в каноне — KPI-карточки и строки), даты-календарь как виджет. Эти паттерны противоречат мобильной капсуле 480px; запрос — через владельца.

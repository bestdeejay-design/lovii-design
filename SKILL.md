...
5. `examples/01…05.html` — **референс-экраны по ролям** (готовые страницы, обе темы)
6. `docs/08-recipes.md` — пошаговые рецепты типовых задач (новый экран, новый токен, приёмка…)
7. `style-guide.html` — визуальный эталон (открыть в браузере, переключить тему)
8. `docs/checklist.md` — приёмка перед завершением

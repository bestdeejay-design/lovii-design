...
## 8. LOVII PAY — профиль лояльности (v1.13.2)
...
**Композиция экрана (v1.13.2, адаптив):** контейнер `.lv-narrow` (max 760px). Мобильный (<640px): один столбец в порядке карта → счёт → история → статус → привилегии → МСП. От **≥640px** — двухколоночная сетка `.pay-cols` (5fr/6fr) «как в веб-банке»: слева карта + счёт (`.pay-col-a`, sticky top 68px при высоте окна ≥660px), справа история + статус (`.pay-col-b`); привилегии — сеткой `auto-fit minmax(200px,1fr)`, от 1024px — промежуток 28px. Порядок секций на мобильном не меняется — колонки стекаются.

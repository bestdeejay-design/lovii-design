...
| `otp-input` | поле ввода SMS-кода с авто-подстановкой (вход, подтверждение действий; клиентское приложение и B2B — один стандарт) | ОДИН инпут: `type=text` · `inputmode=numeric` · `autocomplete=one-time-code` · `name=otp` · `maxlength` из `otp.code_length` · фокус при монтировании; автосабмит на длине; хук WebOTP и формат SMS (`@домен #код`) — docs/10 §5; split-ячейки запрещены | `--lv-card`, `--lv-line`, `--lv-ink`, `--lv-dim` |

Правила использования: `lock-screen` — единственный полный экран-перехват в продукте (безопасность); все прочие подтверждения — bottom-sheet. Face ID-строка скрыта (не disabled), если платформа не поддерживает WebAuthn platform authenticator. PIN живёт только на устройстве: `SHA-256(соль + PIN)` в localStorage, голый PIN и бэкенд не участвуют. Экран-перехват обязан влезать в вьюпорт целиком: на складных/квадратных экранах (обложки флипов, flex-половина — вьюпорты в docs/04 «Брейкпоинты») при `max-height: 659px` включается компакт-режим; точка схода обычного режима — та же 660px, что и у sticky-колонок профиля. `otp-input` — по docs/10 §5 без вариаций (клиент + B2B).

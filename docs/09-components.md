...
| Движение | flip `rotateY(180deg)` по тапу, .65s cubic-bezier(.2,.75,.25,1) · tilt по курсору ±10°/±8° · блик `.pay-glare` — световое пятно следит за курсором через `--gx/--gy` (только `pointer: fine`; off при reduce-motion) · блик `.pay-sweep` (v1.14.1) — живой свип-луч: диагональная полоса света (115°) пробегает через карту при hover/tap за ~1.15s cubic-bezier(.25,.6,.3,1); луч тонирован под скин; `transition: none` в покое — при уходе курсора луч мгновенно сбрасывается, каждое наведение = свежий пробег; off при reduce-motion | `aspect-ratio: 1.586/1`, радиус `--lv-r-xl`, тень `--lv-shadow-nav`; на hover тень глубже (`@media hover:hover`) |
...
Блики (v1.14.1): `.pay-glare` — пятно за курсором; `.pay-sweep` — живой свип-луч через карту (эффект прототипа `lv-card-shine`, канон владельца «очень живой эффект»): тон луча под скин — серебро (PAY), розовый (PASS), золотой (VIP), тиффани (BUSINESS). Луч — спан в обеих сторонах карты.

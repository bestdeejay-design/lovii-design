...
## Тихая тёмная тема (принцип v1.9)

Тёмная тема — не инверсия светлой, а другой жанр: тихий, дорогой,
с одним голосом. Брендовые значения pink/tiffany/gold не меняются (docs/02),
но ПЛОТНОСТЬ цвета на тёмном ниже — насыщенные тона на тёмном «выступают вперёд».

1. **Один акцент в кадре** — pink: CTA, ссылки, активный пункт навигации, фокус,
   итоговые строки. Это единственный громкий голос темы.
2. **Данные — цветом чернил.** Крупные числа, метрики, значения в карточках —
   `--lv-ink`, а не акцентами. Три цвета цифр в одном ряду = пестрота (урок lovii.ru v2.0).
3. **Вторичные акценты — точечно.** tiffany/gold на тёмном живут в микрометках:
   семантика фаз таймлайна, статус «успех», единичные кольца декора. Не — в пило́ли,
   заголовки и массовые чипы.
4. **Декор почти монохром.** Кольца/волны на тёмном — альфы вдвое ниже светлого,
   одно семейство с фоном; пульс-точки — одна гамма. Розовое свечение hero — можно.
5. **Проверка прищуром.** Сфоткай экран, прищурься: если цветовых пятен больше трёх —
   тема кричит, убери цвет у данных и декора.

Тексты-пары (`--lv-tiffany-text`/`--lv-gold-text`) в v1.9 приглушены до пыльных
оттенков — контраст сохранён (≥ 8:1), неон уходит (tokens.css, dark-блок).
...
- [ ] Тихая тёмная тема: один акцент в кадре, данные — чернилами, декор монохромный (v1.9)

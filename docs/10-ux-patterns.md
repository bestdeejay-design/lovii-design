...
> **Актуальность (решение владельца 2026-09-13):** SMS-канал в платформе НЕ планируется —
> боевой канал входа MAX (код 4 цифры, `OtpChannel::codeLength()`), SMS-сендер в lovii-core
> помечен LEGACY-заглушкой. Паттерн §5 остаётся каноном на случай возврата SMS; применять
> сегодня: атрибуты поля (§5.1) и **длина поля — всегда из канала доставки, не хардкод**
> (инцидент b2b-staging: input просил 6 цифр при боевом MAX-коде в 4 — фикс d639b29,
> `codeLength()` из enum + `digits:{N}`-валидация + placeholder/maxlength из длины).
> MAX-бот доставляет код сообщением — автоподстановка из SMS неприменима в принципе;
> UX-компенсация: автофокус, автосабмит на длине, крупный пад.

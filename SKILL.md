...
8. `docs/13-site-longform.md` — **сайт продукта и лонгрид**: каркас lovii.ru, hero-подпись, ленточные паттерны (toc/highlight/stats/timeline/calc/data-table), декор, SEO/печать
8. `examples/01…08.html` — **референс-экраны по ролям + UX-лаборатория + Тон-лаборатория + Библиотека компонентов** (готовые страницы, обе темы)
